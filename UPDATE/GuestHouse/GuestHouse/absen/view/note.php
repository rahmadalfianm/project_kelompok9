<?php
    if (isset($_SESSION['pb'])) {
        ...
        echo "<h3 class='sub-header'>Catatan Kegiatan <button type='button' onclick=\"window.location.href='tambah_catatan';\" class='btn btn-success'> Tambah Kegiatan</button> </h3>";
	    ...
	    if (isset($_GET['st'])) {
	      if ($_GET['st']==1) {
	        echo "<div class='alert alert-success'><strong>Berhasil disimpan.</strong></div>";
	      } else {
	        echo "<div class='alert alert-danger'><strong>Gagal menyimpan.</strong></div>";
	      }
	    }
	        if ($conn->query("SELECT*FROM catatan WHERE id_user='$id_user'")->num_rows!==0) {
	            ...
	            while ($get_month=$query_month->fetch_assoc()) {
	              ...
	              $cek = $query_note->num_rows;
	              if ($cek!==0) {
	                echo "<h4 class='sub-header'>Kegiatan bulan $month </h4>";
	                echo "<div class='table-responsive'>
	                   <table class='table table-striped'>
	                    <thead>
	                       <tr>
	            ...
	            echo "<div class='alert alert-warning'><strong>Tidak ada kegiatan untuk ditampilkan.</strong></div>";
	        }
    }
?>
